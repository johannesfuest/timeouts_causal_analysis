...
df <- df %>%
  ...
  mutate(
    ...
    HomeTeamTimeout = ifelse(TimeoutCalledInitially, ifelse(grepl("timeout", first(AwayEvent)) | (StartDrb & (grepl("timeout", AwayEvent[2]))), 0, 1), NA),
    TimeoutTime =  ifelse((StartDrb & (grepl("timeout", AwayEvent[2]) | grepl("timeout", HomeEvent[2]))), Seconds[2], StartPossTime),
...
df <- filter(df, is.na(HomeTeamTimeout) | ((!(!HomePoss & HomeTeamTimeout)) & (!(HomePoss & !HomeTeamTimeout))))
...
summary_df <- df %>%
  ...
  summarise(
    ...
    HomeTeamTimeout = first(HomeTeamTimeout),
    StartPossTime = first(StartPossTime),
...
home_off <- filter(off_def_ratings) %>%
  ...
  ungroup()
away_off <- filter(off_def_ratings, !is_home) %>%
  group_by(ESPNGameID) %>%
  summarise(
    off_rating = mean(off_rating[!is_home]),
    def_rating = mean(def_rating[is_home]),
